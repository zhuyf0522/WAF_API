...
class GetWafVersion(unittest.TestCase):
    ...
    def testWafGeneral(self):
        ...
        if 'HostNames' in general_value:
            v_HostNames=general_value.get('HostNames')
        else:
            print('ERROR: No HostNames in parameter list , this testcase will be exit.')
            exit()
        v_WebServerPort=general_value.get('WebServerPort',default=80)
        v_ListenPort=general_value.get('ListenPort', default=80)
        v_BindIP=general_value.get('BindIP',default=None)
        v_ListenIP=general_value.get('ListenIP',default=None)
        v_Bypass=general_value.get('Bypass', default=False)
